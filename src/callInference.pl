/*************************************************************************
    ...
    It is adapted from the file callInference.pl of BB1, version 1.3
...
:- module(callInference,[infix/0,
                         ...
                         mbTestSuite/0,
                         tpTestSuite/0,
                         tpmbTestSuite/0,initTheoremProvers/2]).
...
initTheoremProvers(A,B) :-
   inferenceEngines(Engines),
   initTheoremProvers(Engines,A,B).

initTheoremProvers([],_,_).

initTheoremProvers([Prover|L], Formula, Info) :-
   ((Info = '', !,Sep = ''); (Sep = '-')),
   concat_atom([tpinput,'/',Prover,Sep,Info,'.in'],FileName),
   %string_concat(Prover,'.in', FileName),
   open(FileName, write, Stream),
   proverCall(Prover,Formula,Stream),
   close(Stream),
   initTheoremProvers(L, Formula, Info).
...
initTheoremProvers([E|L],Formula,Info):-
   write('initTheoremProvers/2: '),write(E),nl,
   initTheoremProvers(L,Formula,Info).


proverCall(otter, Formula, Stream) :-
   fol2otter(not(Formula),Stream),!.
proverCall(bliksem, Formula, Stream) :-
    fol2bliksem(not(Formula), Stream),!.
proverCall(prover9,  Formula, Stream) :-
   fol2prover9(Formula, Stream),!.
proverCall(eprover, Formula, Stream) :-
   fol2tptp:fol2tptp(Formula, Stream, eprover),!.
proverCall(spass, Formula, Stream) :-
   fol2tptp:fol2tptp(Formula, Stream, spass),!.
proverCall(A, Formula, Stream) :-
    write(proverCall(A, Formula, Stream)),nl.
...
initModelBuilders([mace|L],Formula):- !,
   ...
   fol2prover9(Formula,Stream), %fol2mace
   close(Stream),
...
callTP(Problem,Proof,Engine,Parameters):-
   executeCommand('chmod 700 interfaceTP.perl'),
   inferenceEngines(Engines),
   initTheoremProvers(Engines,Problem,''),
   buildCommandString(CommandString,Parameters),
   concatStrings([CommandString|Engines],Shell),        
   ...
   close(Out).
callTP(A,B,C,D) :-
   write(callTP(A,B,C,D)). %(mhahn)

% mhahn
buildCommandString('perl ./interfaceTP.perl ',[]).
buildCommandString(Command,[timeout:Timeout]) :-
   concatStrings(['perl ./interfaceTP.perl ','--timeout=',Timeout,' '],Command).
...
callMB(Problem,DomainSize,Model,Engine):-
   ...
   (
      ...
      read(Out,engine(Engine)),close(Out),
...
mace2blackburnbos(Mace,model(D,F)):- %trace,
   Mace = interpretation(Size,Info,Terms),
   mace2d(1,Size,D),
   mace2f(Terms,D,F).
...
mace2f([relation(Relation,V)|Terms],D,[f(1,Functor,X)|F]):-
	Relation =.. [Functor,_], !,
	positiveValues(V,1,X),
	mace2f(Terms,D,F).

mace2f([relation(Relation,V)|Terms],D,[f(2,Functor,X)|F]):-
